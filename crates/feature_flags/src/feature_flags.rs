...
use gpui::{AppContext, Global, ModelContext, Subscription};
use std::{
    future::Future,
    pin::Pin,
    task::{Context, Poll},
...
pub trait FeatureFlagViewExt<V: 'static> {
    ...
    where
        F: Fn(bool, &mut V, &mut ModelContext<V>) + Send + Sync + 'static;
}

impl<V> FeatureFlagViewExt<V> for ModelContext<'_, V>
where
    V: 'static,
{
    fn observe_flag<T: FeatureFlag, F>(&mut self, callback: F) -> Subscription
    where
        F: Fn(bool, &mut V, &mut ModelContext<V>) + 'static,
    {
